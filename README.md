...
- **Advanced Analytics**: Integrates OpenGolfCoach for comprehensive derived shot metrics
...
## Advanced Analytics (OpenGolfCoach Integration)

This integration leverages the powerful OpenGolfCoach library to provide a wealth of derived golf shot metrics. When a shot message is received, the raw data is fed into OpenGolfCoach, which calculates additional values such as:

*   **Carry Distance (Metric & US Customary)**
*   **Total Distance (Metric & US Customary)**
*   **Offline Distance (Metric & US Customary)**
*   **Peak Height (Metric & US Customary)**
*   **Descent Angle**
*   **Hang Time**
*   **Club Speed**
*   **Smash Factor**
*   **Calculated Backspin, Sidespin, Total Spin, Spin Axis**
*   **Club Path, Club Face to Target, Club Face to Path**
*   **Shot Classification (Name, Rank, Color)**

These derived values are exposed as additional sensors in Home Assistant, allowing for more detailed analysis and dashboard visualizations.

## Sensors

### Shot Data (updated on each shot)
(Includes all raw data from the NOVA device, plus many derived metrics from OpenGolfCoach)
...
A sample Lovelace dashboard configuration (`nova_dashboard.yaml`) is provided to help visualize the raw and derived golf shot data.

1.  **Place the Dashboard File**: Copy the `nova_dashboard.yaml` file to your Home Assistant `config/lovelace/` directory.
2.  **Add to Home Assistant**:
    *   In Home Assistant, navigate to `Settings` -> `Dashboards`.
    *   Click the `+ ADD DASHBOARD` button.
    *   Choose "YAML" mode and either paste the content of `nova_dashboard.yaml` or import the file if prompted.
3.  **Adjust Entity IDs**: The provided `nova_dashboard.yaml` uses entity IDs like `sensor.nova_launch_monitor_ball_speed`. **You MUST adjust these entity IDs** within the `nova_dashboard.yaml` file to match the actual entity IDs generated by your Home Assistant installation for your Nova device (e.g., if your device name is "My Nova", the entity ID might be `sensor.my_nova_ball_speed`).
4.  **Install Custom Cards (if needed)**: The dashboard utilizes custom Lovelace cards (e.g., `vertical-stack-in-card`, `button-card`). If you don't have these, install them via HACS (Home Assistant Community Store) for the dashboard to render correctly.

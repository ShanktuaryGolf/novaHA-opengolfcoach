    ...
    async def _process_message(self, message: str) -> None:
        ...
        try:
            ...
            if msg_type == "shot":
                ...
                # Prepare data for opengolfcoach
                opengolfcoach_input = {
                    "ball_speed_meters_per_second": data.get("ball_speed"),
                    "vertical_launch_angle_degrees": data.get("launch_angle"),
                    "horizontal_launch_angle_degrees": data.get("launch_direction"),
                    "total_spin_rpm": data.get("total_spin"),
                    "spin_axis_degrees": data.get("spin_axis"),
                    "backspin_rpm": data.get("back_spin"),
                    "sidespin_rpm": data.get("side_spin"),
                    "club_speed_meters_per_second": data.get("club_speed"),
                    "club_path_degrees": data.get("club_path"),
                    "club_face_to_target_degrees": data.get("club_face_angle"),
                    "club_face_to_path_degrees": data.get("club_face_to_path"),
                }

                try:
                    # Convert input to JSON string and call opengolfcoach
                    json_input_str = json.dumps(opengolfcoach_input)
                    derived_values_json_str = opengolfcoach.calculate_derived_values(json_input_str)
                    derived_values = json.loads(derived_values_json_str)
                    
                    # Merge derived values into shot data
                    # opengolfcoach returns a nested 'open_golf_coach' key
                    if "open_golf_coach" in derived_values:
                        data.update(derived_values["open_golf_coach"])

                except Exception as e:
                    _LOGGER.error("Error calling opengolfcoach: %s", e)
